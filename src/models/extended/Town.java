...
import coefficients.SeparationFloorBuilding;

import models.Mesh;
import utils.Algos;
import utils.Algos.NoFloorException;
...
public class Town {
	...
	private ArrayList<Edifice> residentials = new ArrayList<Edifice>();
	private ArrayList<Edifice> industrials = new ArrayList<Edifice>();
	...
	public void addIndustrial(Edifice building) {
		if (!this.industrials.contains(building))
			this.industrials.add(building);
	}

	public void addResidential(Edifice building) {
	...
	private void recollerCorrectementLesSolsAuxMurs(Edifice e,
	...
	private void treatIndustrials(String directoryName, Vector3d normalFloor) {
		...
		// while (new File(directoryName + "industrial - " + counterIndustrial
		// + ".stl").exists()) {
		//
		// long time = System.nanoTime();
		//
		// Mesh mesh = this.parseFile((directoryName + "industrial - "
		// + counterIndustrial + ".stl"));
		//
		// System.out.println("Temps écoulé pour le parsing : "
		...
		// // walls, forms on the ground, maybe chimneys...
	...
	private void treatResidentials(String directoryName, Vector3d normalFloor) {
		...
		while (new File(directoryName + "residential - " + counterResidentials
			...
			try {
				wholeFloor = this.floorExtraction(mesh, normalFloor);
				wholeFloor.writeSTL("floorsBeforeNoiseTreatment.stl");

				buildings = this.buildingsExtraction(mesh, noise);

				ArrayList<Mesh> floors = this.noiseTreatment(wholeFloor, noise);
				wholeFloor.writeSTL("floorsAfterNoiseTreatment.stl");

				int counter = 0;
				for (Mesh m : buildings) {
					m.writeSTL("building - " + counter + ".stl");
					counter++;
				}
				System.exit(0);
	...
	public void writeSTLIndustrials() {
		...
		for (Edifice m : this.industrials) {
	...
	public void writeSTLResidentials() {
		...
		for (Edifice m : this.residentials) {
			m.writeSTL("Files/building - " + buildingCounter + ".stl");
			buildingCounter++;
		}
	}
