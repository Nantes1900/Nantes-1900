...
public class Building {
     ...
     */
    private static List<Surface> sortRoofs(final TriangleMesh building,
	    final Vector3d normalGround, final TriangleMesh noise) {
     ...
     */
    private static List<Surface> sortWalls(final TriangleMesh building,
	    final Vector3d normalGround, final TriangleMesh noise) {
	...
	// Building.searchForNeighbours(wallTreatedList, roofTreatedList,
	// grounds);
	//
    ...
    private static void treatNoise(final List<Surface> wallList,
	    final List<Surface> roofList, final TriangleMesh noise, final TriangleMesh grounds) {
    ...
    /*
     * tempBuilding = new Mesh(building); // Removes the files if they exist, to
     * avoid confusion between // old and new files. int counterWall = 1; int
     * counterRoof = 1; } // Creates a new mesh. noise = new Mesh(); // Applies
     * the first algorithms : extract the walls, and after this, // extract the
     * roofs. wallList = Building.sortWalls(tempBuilding, normalGround, noise);
     * roofList = Building.sortRoofs(tempBuilding, normalGround, noise); //
     * Treats the noise. Building.treatNoise(wallList, roofList, noise,
     * groundsTreated); // Finds the neighbours, searching deeply (add noise to
     * the walls // and roofs to add every edge to one of these surface. Then
     * find // the surfaces which share one edge, and treat the neighbours.
     * Building.treatNewNeighbours(wallList, roofList, groundsTreated); if
     * (Town.stepByStep) { System.out
     * .println("2nd step executed : you will find the result in : " +
     * directoryName + FilesNames.TEMPORARY_DIRECTORY +
     * FilesNames.RESIDENTIAL_FILENAME + FilesNames.SEPARATOR + counterZone +
     * FilesNames.SEPARATOR + FilesNames.BUILDING_NAME + FilesNames.SEPARATOR +
     * counterBuilding); int counterWall = 1; for (final Mesh m : wallList) {
     * m.writeSTL(directoryName + FilesNames.TEMPORARY_DIRECTORY +
     * FilesNames.RESIDENTIAL_FILENAME + FilesNames.SEPARATOR + counterZone +
     * FilesNames.SEPARATOR + FilesNames.BUILDING_NAME + FilesNames.SEPARATOR +
     * counterBuilding + FilesNames.SEPARATOR + FilesNames.WALL_NAME +
     * FilesNames.SEPARATOR + counterWall + FilesNames.EXTENSION);
     * counterWall++; } int counterRoof = 1; for (final Mesh m : roofList) {
     * m.writeSTL(directoryName + FilesNames.TEMPORARY_DIRECTORY +
     * FilesNames.RESIDENTIAL_FILENAME + FilesNames.SEPARATOR + counterZone +
     * FilesNames.SEPARATOR + FilesNames.BUILDING_NAME + FilesNames.SEPARATOR +
     * counterBuilding + FilesNames.SEPARATOR + FilesNames.ROOF_NAME +
     * FilesNames.SEPARATOR + counterRoof + FilesNames.EXTENSION);
     * counterRoof++; } } } while (Town.stepByStep && !Town.askForAnswer()); do
     * { if (Town.stepByStep) { // Loads the new coefficients from the config
     * file. Configuration.loadCoefficients(); this.walls.clear();
     * this.roofs.clear(); } // From all the neighbours, computes the wrap line
     * and returns the // surfaces as polylines.
     * this.findEdgesFromNeighbours(wallList, roofList, normalGround,
     * groundsTreated, noise); if (Town.stepByStep) { System.out
     * .println("3rd step executed : you will find the result in : " +
     * directoryName + FilesNames.TEMPORARY_DIRECTORY +
     * FilesNames.RESIDENTIAL_FILENAME + FilesNames.SEPARATOR + counterZone +
     * FilesNames.SEPARATOR + FilesNames.BUILDING_NAME + FilesNames.SEPARATOR +
     * counterBuilding); this.writeSTL(directoryName +
     * FilesNames.TEMPORARY_DIRECTORY + FilesNames.RESIDENTIAL_FILENAME +
     * FilesNames.SEPARATOR + counterZone + FilesNames.SEPARATOR +
     * FilesNames.BUILDING_NAME + FilesNames.SEPARATOR + counterBuilding); } }
     * while (Town.stepByStep && !Town.askForAnswer()); // Recompute the bounds
     * between the grounds and the buildings, to avoid // holes. //
     ...
     */
    private final List<Roof> roofs = new ArrayList<Roof>();
     ...
     *            the roof to add
     */
    public final void addRoof(final Roof roof) {
	this.roofs.add(roof);
    }
     ...
     */
    public final void addRoofs(final List<Roof> addRoofs) {
     ...
     *            the wall to add
     */
    public final void addWall(final Wall wall) {
	this.walls.add(wall);
    }
     ...
     *            the wall to add
     */
    public final void addWalls(final List<Wall> addWalls) {
    ...
    public final void findEdgesFromNeighbours(final List<Surface> wallList,
	    ...
	    final Surface grounds, final TriangleMesh noise) {
	...
	// }
	//
	// Town.LOG.finest("Surfaces correctly treated : " +
	// counterWellTreated);
    ...
    private void reComputeBounds(final TriangleMesh grounds) {
